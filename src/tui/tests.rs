...
#[test]
fn test_phase9_1_edit_buffer_policy_numeric_preserved_secret_cleared() {
    let mut model = Model::new();

    // 1. City search uses CleanReplacement
    model.active_tab = Tab::Location;
    model.active_setting = 0;
    assert_eq!(
        model.edit_buffer_policy(),
        super::app::EditBufferPolicy::CleanReplacement
    );

    // 2. Settings API key uses CleanReplacement
    model.active_tab = Tab::Settings;
    model.active_setting = super::model::settings_index::WEATHER_API_KEY;
    assert_eq!(
        model.edit_buffer_policy(),
    ...
    // 3. Numeric & text fields use ExistingValue
    model.active_tab = Tab::Settings;
    model.active_setting = super::model::settings_index::REFRESH_RATE;
    assert_eq!(
        model.edit_buffer_policy(),
        super::app::EditBufferPolicy::ExistingValue
    );
    ...
    model.active_tab = Tab::Location;
    model.active_setting = 1; // Latitude
    assert_eq!(
        model.edit_buffer_policy(),
        super::app::EditBufferPolicy::ExistingValue
    );

    model.active_setting = 3; // Timezone
    assert_eq!(
        model.edit_buffer_policy(),
        super::app::EditBufferPolicy::ExistingValue
    );

    // 4. Behavioral test: FPS preserves value on enter
    model.active_tab = Tab::Settings;
    model.active_setting = super::model::settings_index::REFRESH_RATE;
    model.form.fps_input = tui_input::Input::default().with_value(String::from("60"));
    model.start_editing();
    ...
    // 5. Behavioral test: API key starts empty and snapshot preserves original
    model.active_setting = super::model::settings_index::WEATHER_API_KEY;
    model.form.api_key_input =
        tui_input::Input::default().with_value(String::from("existing_secret_123"));
    model.start_editing();
    assert_eq!(model.form.api_key_input.value(), ""); // Clean start!
    model.cancel_editing();
    assert_eq!(model.form.api_key_input.value(), "existing_secret_123"); // Restored!
}
